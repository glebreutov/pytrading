...
class HedgeConfig:
    def __init__(self, d):
        ...
        self.ema_work_perc = self.ema_work_perc = Decimal(d['ema_work_perc'])
...
def bound_to_lower_quote(quote: Level, pos: Position, tick_size):
    def price_equlas_quote():
        for x in quote:
            if x.price == pos.price:
                return True
        return False
    ...
    try:
        if pos.side() == Side.BID:
            stick_price = max([x.price for x in quote if x.price < pos.price()]) + tick_size
        else:
            stick_price = min([x.price for x in quote if x.price > pos.price()]) - tick_size
        return Position(pos=pos.pos, side=pos.side(), price=stick_price)
    except ValueError:
        return pos
...
def enter_hedge(pnl: PNL, quote: Level, cfg: HedgeConfig, vc: VenueConfig):
    ...
    if pos.abs_position() < vc.min_order_size:
        ...
        # exit order
        # depth or zero
        return exit_order(quote, pos, cfg.liq_behind.side(side), vc)
